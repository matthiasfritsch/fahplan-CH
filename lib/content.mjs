...
/* Flohmarkt als eigene Kategorie: "category": "flohmarkt" in der
   Liste, oder am Titel erkannt (Flohmi, Flohmarkt, Brocante, Troedel). */
export function isFleaMarket(e) {
  return e.category === "flohmarkt" ||
    /floh|brocante|troedel|tr\u00f6del/i.test(e.title || "");
}

/* Prioritaet: Termine von Yunas Schule vor Flohmaerkten vor dem
   Rest. "prio": true hebt ein beliebiges Event auf Flohmarkt-Stufe. */
export function weight(e) {
  if (e.tip) return 0.5;              // Tipps nur als Lueckenfueller
  if (e.school) return 3;
  if (e.prio || isFleaMarket(e)) return 2;
  return 1 + Math.max(-0.9, Math.min(0.9, e.score || 0));
}
...
/* Waehlt aus, was auf die Tafel kommt.
   ...
   - Unter der Woche hoechstens ein Event pro Tag, heute zwei.
     Schultermine und Flohmaerkte haben Vorrang (siehe weight).
...
export function selectEvents(events, today, nowTime, budget = EVENT_BUDGET, holidays = [], { ideas = [], rainy = false } = {}) {
  ...
  const weekEnd = end;   // Tipps nur bis zu diesem Sonntag
  ...
  /* Wochenend- und Ferientage mit weniger als zwei Events mit
     Tipps auffuellen, damit das Board nie duenn aussieht. Die Tipps
     rotieren nach Datum; bei Regen heute zuerst Drinnen-Tipps. */
  if (ideas.length) {
    const used = new Set();
    for (let d = today; d <= weekEnd; d = addDays(d, 1)) {
      const w = weekday(d);
      const weekendDay = w === 0 || w === 6;
      if (!(weekendDay || isHoliday(d))) continue;
      const list = byDay.get(d) || [];
      const want = weekendDay ? 2 : 1;   // Ferien-Werktage: ein Tipp reicht
      let pool = ideas.map((x, i) => ideas[(i + dayNumber(d)) % ideas.length]);
      if (d === today && rainy) pool = [...pool.filter(x => x.indoor), ...pool.filter(x => !x.indoor)];
      for (const idea of pool) {
        if (list.length >= want) break;
        if (used.has(idea.title)) continue;
        used.add(idea.title);
        list.push({ date: d, time: "", title: idea.title, place: idea.place, tip: true });
      }
  ...
  let days = [...byDay.keys()].sort().map(date => {
    ...
    // Wichtige zuerst behalten (Schule, Flohmarkt), dann nach Zeit
    // sortiert anzeigen.
    const list = byDay.get(date)
      .sort((a, b) => weight(b) - weight(a) || sortKey(a) - sortKey(b))
      .slice(0, cap)
  ...
  while (height() > budget && days.length) {
    // Zuerst Tipps streichen, von hinten
    const withTip = [...days].reverse().find(d => d.list.some(e => e.tip));
    if (withTip) {
      const i = withTip.list.map(e => !!e.tip).lastIndexOf(true);
      withTip.list.splice(i, 1);
      if (!withTip.list.length) days = days.filter(d => d !== withTip);
      continue;
    }
    ...
    if (fullest.list.length > 1) {
      // Das unwichtigste Event des Tages streichen, nicht das spaeteste
      const drop = [...fullest.list].sort((a, b) => weight(a) - weight(b) || sortKey(b) - sortKey(a))[0];
      fullest.list = fullest.list.filter(e => e !== drop);
    }
    else days.pop();
  }
